...
def pod_selector_labels(marathon_app_id: str) -> Mapping[str, str]:
    ...
    return {'app': utils.make_label(app_id)}
...
def translate_multitenancy(fields: Dict[str, Any]) -> Translated:
    ...
    selector_labels = pod_selector_labels(marathon_app_id)
    return Translated(
        update={
            'metadata': {
...
def generate_root_mapping(
    ...
    return {
        ...
        ('container', ): get_container_translator(container_defaults, app_secrets_mapping, error_location),
...
def get_container_translator(
    ...
    def translate_container(fields: Mapping[str, Any]) -> Translated:
        update, warnings = apply_mapping(mapping={
            ...
            skip_quietly,
            "volumes":
            lambda _: translate_volumes(_, app_secret_mapping),
            "type":
            skip_quietly,
        },
...
def translate_app(app: Dict[str, Any], settings: Settings) -> TranslatedApp:
    ...
    def register_node_labels(labels: Set[str]) -> None:
        return node_labels.update(labels)
