...
def main():
    ...
    backup_cmd.add_argument("--snapshot-name", type=str, default="", help="Snapshot or Backup Name (no default, if --only-conf=False, it would be required)")
    backup_cmd.add_argument("--bucket-name", type=str, default="", help="S3 Bucket Name (no default, if --only-conf=False, it would be required)")
    backup_cmd.add_argument("--keyspaces", type=str, default="", help="Comma separated list of keyspace names for the Backup (no default, optional)")
    backup_cmd.add_argument("--aws-key", type=str, default="", help="AWS Access Key ID (no default, if --only-conf=False, it would be required)")
    backup_cmd.add_argument("--aws-secret", type=str, default="", help="AWS Secret Access Key (no default, if --only-conf=False, it would be required)")
    backup_cmd.add_argument("--aws-session-id", type=str, default="", help="AWS Session ID (no default, optional)")
    backup_cmd.add_argument("--aws-session-token", type=str, default="", help="AWS Session Token (no default, optional)")
    ...
    # Step 3 : Migrate the schema and data from DC/OS Cassandra to Kubernetes Cassandra
    migrate_cmd = subparsers.add_parser("migrate",
                                        help='Restore the Schema and Data from the backup of MesosCloud based Cassandra to KubernetesCloud based Cassandra')
    ...
    migrate_cmd.add_argument("--bucket-name", type=str, required=True, help="S3 Bucket Name")
