...
RESTORE_SCRIPT = os.path.join(
    ...
    "../resources/dcos_cassandra_restore.sh")
...
def _generate_restore_cmd(instance: str, node_index: int, namespace: str, restore_params: dict) -> str:
    ...
    cmd += "--bucket-name {} --snapshot {} --aws-region {} --aws-key {} --aws-secret {}".format(
        restore_params["S3_BUCKET_NAME"],
        restore_params["SNAPSHOT_NAME"],
        restore_params["AWS_REGION"],
        restore_params["AWS_ACCESS_KEY_ID"],
        restore_params["AWS_SECRET_ACCESS_KEY"])
...
def restore_schema_and_data(namespace: str, instance: str, node_count: int, restore_params: dict):
    ...
    run_cmd(
        "{} kudo plan status --namespace={} --instance={}".format(
            KUBECTL, namespace, instance), check=True)
    ...
    for node_index in range(node_count):
        ...
        rc, _, _ = run_cmd("{} cp {} {}/{}:/tmp/".format(
            KUBECTL, RESTORE_SCRIPT, namespace, pod_name))
        if rc != 0:
        ...
        rc, _, _ = run_cmd("{} exec --namespace={} {} -c cassandra -- {}".format(
            KUBECTL, namespace, pod_name, restore_cmd))
        if rc != 0:
